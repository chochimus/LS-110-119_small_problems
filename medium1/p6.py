...
Your task is to programmatically determine whether a number is prime without relying on 
functions that already do that for you.

input: positive integer
ouput: boolean if number is prime

rules:
    explicit:
    - no add on pakages to solve
    implicit:
    - expect good input
...
2.) if divisible return false else increment and check
3.) repeat for digits up to sqrt of input number
4.) return result
"""
...
def is_prime(digit):
    ...
    for num in range(2,int(math.sqrt(digit)) + 1):
        if digit % num == 0:
            return False
    return True
