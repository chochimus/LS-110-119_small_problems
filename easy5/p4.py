"""
Problem:
Write a function that takes a string as an argument and returns a list that contains every word 
...
input: string
output: return list that contains every word from string followed by a space and words length

rules:
    explicit:
    - list consist of word followed by length separated by space
    - empty string or no argument should return empty list
    - assume every pair of words will be separated by a single space
    implicit:
    - expect valid input
...
2.) add word and word length to answer separated by space
3.) repeat for every word in words
4.) return answer
"""
