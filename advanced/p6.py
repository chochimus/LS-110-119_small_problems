...
With a linear search, we would have to sequentially go through each of 
the names until we found the search item, 'Smith'. In a binary search, 
...
Repeat the process from the top, using the upper 
half as the starting data --> ['Lee', 'Seeli', 'Smith', 'Zimmer'].
...
third iteration when the middle value is 'Smith'. For an 8-element list like this,
we need, at most, 3 iterations. For an 8000-element list, we need, at most,
just 13 iterations. For 8,000,000 elements, we need just 23 iterations. 
...
algorithm:
1.) check if midpoint == value
2.) if value is less repeat search on lower half
3.) if value is more repeat search on upper half
4.) repeat until value is found or list is empty
"""
