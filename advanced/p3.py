"""
Problem:
As we saw in the previous exercises, a matrix can be represented by a list of lists. 
For example, the 3x3 matrix shown below:

1  5  8
4  7  2
3  9  6
is represented by the following list of list:

matrix = [
    [1, 5, 8],
    [4, 7, 2],
    [3, 9, 6],
]
A 90-degree rotation of a matrix produces a new matrix in which each side of 
the matrix is rotated clockwise by 90 degrees. For example, the 90-degree 
rotation of the matrix shown above is:

3  4  1
9  7  5
6  2  8
A 90-degree rotation of a non-square matrix is similar. For example, given the following matrix:


3  4  1
9  7  5
its 90-degree rotation is:

9  3
7  4
5  1
Write a function that takes an arbitrary MxN matrix, rotates it clockwise
by 90-degrees as described above, and returns the result as a new matrix. The function should not mutate the original matrix.

input: 2d list matrix
output: new 2d list = rotated matrix

rules:
    explicit:
    - rotated matrix is a matrix with every side rotated clockwise 90 degrees
    implicit:
    - expect good input

datastructure:
2d list

algorithm:
1.) create empty matrix answer with col length empty lists
2.) starting at last row, first col add element to first element of answer
3.) decrement row
4.) repeat 2-3 until first row
5.) increment col
6.) repeat 2 - 5 until end reached
7.) return answer
"""

def rotate90(matrix):
    return [list(row)[::-1] for row in zip(*matrix)]

matrix1 = [
    [1, 5, 8],
    [4, 7, 2],
    [3, 9, 6],
]

matrix2 = [
    [3, 7, 4, 2],
    [5, 1, 0, 8],
]

new_matrix1 = rotate90(matrix1)
new_matrix2 = rotate90(matrix2)
new_matrix3 = rotate90(rotate90(rotate90(rotate90(matrix2))))

# These examples should all print True
print(new_matrix1 == [[3, 4, 1], [9, 7, 5], [6, 2, 8]])
print(new_matrix2 == [[5, 3], [1, 7], [0, 4], [8, 2]])
print(new_matrix3 == matrix2)

def rotate90_v2(matrix):
    answer = [[] for _ in range(len(matrix[0]))]
    
    for col_idx in range(len(matrix[0])):
        for row_idx in range(len(matrix) - 1, -1, -1):
            answer[col_idx].append(matrix[row_idx][col_idx])
    return answer 

matrix1 = [
    [1, 5, 8],
    [4, 7, 2],
    [3, 9, 6],
]

matrix2 = [
    [3, 7, 4, 2],
    [5, 1, 0, 8],
]

new_matrix1 = rotate90_v2(matrix1)
new_matrix2 = rotate90_v2(matrix2)
new_matrix3 = rotate90_v2(rotate90_v2(rotate90_v2(rotate90_v2(matrix2))))

# These examples should all print True
print(new_matrix1 == [[3, 4, 1], [9, 7, 5], [6, 2, 8]])
print(new_matrix2 == [[5, 3], [1, 7], [0, 4], [8, 2]])
print(new_matrix3 == matrix2)