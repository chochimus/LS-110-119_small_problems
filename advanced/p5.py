...
A merge sort is a recursive sorting algorithm that works by breaking down a list's
 elements into nested sub-lists, then combining those nested sub-lists back 
 together in sorted order. It is best explained with an example. 
 Given the list [9, 5, 7, 1, 8, 2, 0, 6], let's walk through the process of 
 sorting it with merge sort. We'll start off by breaking the list down into nested sub-lists:
...
contains the values from the input list in sorted order. 
...
input: list
output: new list contains values from input list in sorted order

rules:
    explicit:
    - assume every element will have same data type
    - merge sort alg
    - should use merge from last exercise
    implicit:
...
algorithm:
1.) recursively break down lists until they are each one element
2.) run every pair through merge function
3.) repeat step 2 until list is rebuilt
"""

def merge_sort(unsorted):
    if len(unsorted) == 1:
        return unsorted
    sub1 = merge_sort(unsorted[:len(unsorted)//2])
    sub2 = merge_sort(unsorted[len(unsorted)//2:])
